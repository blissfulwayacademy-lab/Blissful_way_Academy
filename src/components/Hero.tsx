...
export function Hero({ onBookTrial }: HeroProps) {
  return (
    ...
    >
      <div className="absolute -left-32 top-8 h-80 w-80 rounded-full bg-gold/10 blur-[120px]" />
      <div className="relative grid items-center gap-14 lg:grid-cols-[1.03fr_.97fr] lg:gap-12">
        <div className="max-w-2xl">
          ...
          <h1 className="font-serif text-[2.9rem] leading-[1.12] tracking-[-0.012em] text-bone sm:text-[3.4rem] lg:text-[4.2rem]">
            Where <span className="text-gold">culture</span> meets
            <br className="hidden sm:block" /> <span className="italic">curiosity.</span>
          </h1>
          <UliRule />
          <p className="mt-6 max-w-xl text-base leading-7 text-bone sm:text-lg">
            Empowering diaspora children with native{' '}
            <span className="font-medium text-gold">Igbo mastery</span> and{' '}
            <span className="font-medium text-gold">early years number confidence</span> through
          ...
          <div className="mt-10 flex items-center gap-3 text-sm text-bone-muted">
            <Sparkles size={15} className="shrink-0 text-gold" />
            <span>
              <strong className="font-semibold text-bone">{COMBINED_YEARS_EXPERIENCE} years</strong>{' '}
